...
import '../../visits/views/add_visit_view.dart';
import '../../visits/views/visit_details_view.dart';
...
class PatientDetailsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<VisitProvider>(
      create: (_) => VisitProvider()..loadVisitsByPatient(patient.patientId),
      child: Consumer<VisitProvider>(
        builder: (context, visitProvider, child) {
          ...
          if (visitProvider.isLoading) {
            ...
            visitChildren = [
              const Divider(height: 1.0, color: AppColors.border),
              ...visitProvider.visits.map((visit) {
                return Column(
                  children: [
                    ListTile(
                      ...
                      subtitle: Text(DateFormat('dd-MMM-yyyy').format(visit.visitDate)),
                      leading: const Icon(Icons.event_note, color: AppColors.primary),
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (_) => VisitDetailsView(visit: visit),
                          ),
                        );
                      },
                    ),
                ...
                IconButton(
                  ...
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => ChangeNotifierProvider<VisitProvider>(
                          create: (_) => VisitProvider(),
                          child: AddVisitView(patient: patient),
                        ),
                      ),
                    ).then((_) {
                      visitProvider.loadVisitsByPatient(patient.patientId);
                    });
                  },
                ),
                IconButton(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildExpansionCard(
                        ...
                        children: _buildPlaceholderChildren('No prescription records found for this patient.'),
                      ),
                      const SizedBox(height: AppDimensions.spaceS),
                      _buildExpansionCard(
                        ...
                        children: _buildPlaceholderChildren('No billing records exist for this patient.'),
                      ),
              ],
            ),
          ),
  ...
  List<Widget> _buildPlaceholderChildren(String message) {
    return [
      const Divider(height: 1.0, color: AppColors.border),
      Padding(
